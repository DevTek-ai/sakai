...
public class SiteAction extends PagedResourceActionII {
    ...
    private static final String STATE_SITE_ADDRESS = "devtek-site-address";
    ...
    private static final String PROP_DEVTEK_SCHOOL = "devtek-school-id";

    public static final String PROP_DEVTEK_DEPARTMENT_TYPE = "devtek-department-type";
    ...
    private String buildContextForTemplate(String preIndex, int index, VelocityPortlet portlet,
        ...
        switch (index) {
            ...
            case 13:
                ...
                String schoolId = getStringAttribute(userDirectoryService.getCurrentUser().getProperties().get(PROP_DEVTEK_SCHOOL), "");

                List<User> allSchoolUsers = userDirectoryService.getUsers();
                allSchoolUsers = allSchoolUsers.stream()
                        .filter(usr -> {
                            String userSchoolId = usr.getProperties().getProperty(PROP_DEVTEK_SCHOOL);
                            return userSchoolId != null && userSchoolId.equals(schoolId);
                        })
                        .collect(Collectors.toList());
                context.put("allusers", allSchoolUsers);
                Map<String, String> propertyCriteria = new HashMap<>();
                propertyCriteria.put(PROP_DEVTEK_SCHOOL, schoolId);
                List<Site> schoolDepatrments = siteService.getSites(SelectionType.MEMBER, null, null, propertyCriteria, SortType.TITLE_ASC, null);
                context.put("allsites", schoolDepatrments);

                context.put(PROP_DEVTEK_SCHOOL, schoolId);
                    ...
                    try {
                        ...
                        try {                                                                //but we still need to handle the possibility of dates coming in from the template.
                            ...
                            if (templateSite.getProperties().getProperty(SITE_UNPUBLISH_DATE) != null && !StringUtils.isBlank(templateSite.getProperties().getProperty(SITE_UNPUBLISH_DATE))) {
                                ...
                                java.lang.NullPointerException n) {    // but if we get an NPE we need to just clear the dates as we're most likely in new site creation from scratch.
                            context.put("readableUnpublishDate", "");    //clear dates
                            context.put("unpublishDate", "");
                            context.put("readablePublishDate", "");
                            context.put("publishDate", "");
    ...
    protected int sizeResources(SessionState state) {
        ...
        if (getBooleanAttribute(state, IS_DISPLAY_ONLY_SCHOOL, false)) {
            if (termProp == null)
                termProp = new HashMap<String, String>();
            termProp.put(PROP_DEVTEK_DEPARTMENT_TYPE, "school");
        } else if (getBooleanAttribute(state, "IS_DISPLAY_ONLY_DEPARTMENT", false)) {
            if (termProp == null)
                termProp = new HashMap<String, String>();
            termProp.put(PROP_DEVTEK_DEPARTMENT_TYPE, "department");
        }
...
//            PortletConfig config = portlet.getPortletConfig();
                    ...
                    for (int i = 0; i < removals.size(); i++) {
                        ...
                        try {
                            ...
                            if (user != null) {
                                ...
                                removeUserPropertiesByEids(Arrays.asList(user.getEid()), Arrays.asList(PROP_DEVTEK_SCHOOL));
                            }
                        } catch (UserNotDefinedException e) {
    ...
    private void readCreateSiteTemplateInformation(ParameterParser params, SessionState state) {
        ...
        try {
            ...
            if (params.getBoolean("copyUsers")) state.setAttribute(STATE_TEMPLATE_SITE_COPY_USERS, Boolean.TRUE);
            else state.removeAttribute(STATE_TEMPLATE_SITE_COPY_USERS);
            if (params.getBoolean("copyContent")) state.setAttribute(STATE_TEMPLATE_SITE_COPY_CONTENT, Boolean.TRUE);
            else state.removeAttribute(STATE_TEMPLATE_SITE_COPY_CONTENT);
            if (params.getBoolean("publishSite")) state.setAttribute(STATE_TEMPLATE_PUBLISH, Boolean.TRUE);
            else state.removeAttribute(STATE_TEMPLATE_PUBLISH);
        } catch (Exception e) {
